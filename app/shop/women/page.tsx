...
const WomenCategoryPage = () => {
  ...
  const sportswearProducts = womenProducts.filter(p => p.subcategory === 'sportswear')
  const shoesProducts = womenProducts.filter(p => p.subcategory === 'shoes')
  ...
  const subcategories = [
    {
      ...
      description: t('shop.women_sportswear_desc', 'Empowering athletic wear for strong women'),
    ...
    },
    {
      ...
      description: t('shop.women_shoes_desc', 'Premium footwear designed for women'),
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-black to-gray-900">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative">
          <div className="text-center">
            ...
            <h1 className="text-4xl md:text-6xl font-bold text-white mb-6 drop-shadow-lg">
              <span className="gradient-text">{t('shop.women', 'Women')}</span> {t('nav.shop', 'Shop')}
            </h1>
            <p className="text-xl text-gray-300 mb-8 max-w-3xl mx-auto leading-relaxed">
              {t('shop.women_hero', 'Empowering athletic wear designed for strong, confident women. From training essentials to competition gear, built for champions.')}
            </p>
            <div className="flex items-center justify-center space-x-8 text-sm text-gray-400">
              <div className="flex items-center space-x-2 bg-white/5 px-4 py-2 rounded-full backdrop-blur-sm">
                ...
                <span>{womenProducts.length}+ {t('shop.products', 'Products')}</span>
              </div>
              <div className="flex items-center space-x-2 bg-white/5 px-4 py-2 rounded-full backdrop-blur-sm">
        ...
        <div className="grid md:grid-cols-2 gap-8">
          {subcategories.map((subcategory) => (
            <Link
              ...
              href={`/shop/women/${subcategory.slug}`}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-20">
        <div className="text-center mb-12">
          ...
          <h2 className="text-3xl md:text-4xl font-bold text-white mb-4">Featured Women's Products</h2>
          <p className="text-gray-300 text-lg max-w-2xl mx-auto">Our most popular women's items, trusted by athletes worldwide</p>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          {womenProducts.slice(0, 8).map((product) => (
            <Link
              key={product.id}
              href={`/shop/women/${product.subcategory}/${product.type.toLowerCase().replace(/\s+/g, '-')}/${product.id}`}
