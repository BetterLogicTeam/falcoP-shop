...
const WomenShoesPage = () => {
  ...
  // Filter products for women shoes
  const womenShoesProducts = products.filter(p => p.category === 'women' && p.subcategory === 'shoes')

  // Get unique types for this subcategory
  const productTypes = Array.from(new Set(womenShoesProducts.map(p => p.type)))
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-black to-gray-900">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative">
          <div className="text-center">
            ...
            <p className="text-xl text-gray-300 mb-8 max-w-3xl mx-auto leading-relaxed">
              {t('shop.women_shoes_desc', 'Premium footwear designed for women. From running to training, built for performance and style.')}
            </p>
            <div className="flex items-center justify-center space-x-8 text-sm text-gray-400">
              <div className="flex items-center space-x-2 bg-white/5 px-4 py-2 rounded-full backdrop-blur-sm">
                ...
                <span>{womenShoesProducts.length}+ {t('shop.products', 'Products')}</span>
              </div>
              <div className="flex items-center space-x-2 bg-white/5 px-4 py-2 rounded-full backdrop-blur-sm">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-16">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {productTypes.map((type) => {
            const typeProducts = womenShoesProducts.filter(p => p.type === type)
            return (
              <Link
                key={type}
                href={`/shop/women/shoes/${type.toLowerCase().replace(/\s+/g, '-')}`}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-20">
        <div className="text-center mb-12">
          ...
          <p className="text-gray-300 text-lg max-w-2xl mx-auto">Our most popular women's shoes</p>
        </div>

        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          {womenShoesProducts.slice(0, 8).map((product) => (
            <Link
              key={product.id}
              href={`/shop/women/shoes/${product.type.toLowerCase().replace(/\s+/g, '-')}/${product.id}`}
