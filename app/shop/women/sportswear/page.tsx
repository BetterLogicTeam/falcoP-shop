...
const WomenSportswearPage = () => {
  ...
  // Filter products for women sportswear
  const womenSportswearProducts = products.filter(p => p.category === 'women' && p.subcategory === 'sportswear')

  // Get unique types for this subcategory
  const productTypes = Array.from(new Set(womenSportswearProducts.map(p => p.type)))
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-black to-gray-900">
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            ...
            >
              ...
              <span className="font-semibold">{t('common.back', 'Back to Women')}</span>
            </Link>
            <div className="flex items-center space-x-2 text-falco-accent font-bold text-xl">
              <StarIcon className="w-6 h-6" />
              <span>{t('shop.sportswear', 'Sportswear')}</span>
            </div>
          </div>
        </div>
      </nav>
      ...
      <div className="relative pt-20 pb-16 overflow-hidden">
        <div className="absolute inset-0 bg-gradient-to-br from-pink-600/5 via-transparent to-purple-800/5"></div>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative">
          <div className="text-center">
            ...
            <h1 className="text-4xl md:text-6xl font-bold text-white mb-6 drop-shadow-lg">
              <span className="gradient-text">{t('shop.sportswear', 'Sportswear')}</span>
            </h1>
            <p className="text-xl text-gray-300 mb-8 max-w-3xl mx-auto leading-relaxed">
              {t('shop.women_sportswear_desc', 'Empowering athletic wear for strong women. From training essentials to competition gear, built for champions.')}
            </p>
            <div className="flex items-center justify-center space-x-8 text-sm text-gray-400">
              <div className="flex items-center space-x-2 bg-white/5 px-4 py-2 rounded-full backdrop-blur-sm">
                ...
                <span>{womenSportswearProducts.length}+ {t('shop.products', 'Products')}</span>
              </div>
              <div className="flex items-center space-x-2 bg-white/5 px-4 py-2 rounded-full backdrop-blur-sm">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-16">
        <div className="text-center mb-12">
          ...
          <p className="text-gray-300 text-lg max-w-2xl mx-auto">Find the perfect sportswear for your activity</p>
        </div>

        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {productTypes.map((type) => {
            const typeProducts = womenSportswearProducts.filter(p => p.type === type)
            return (
              <Link
                key={type}
                href={`/shop/women/sportswear/${type.toLowerCase().replace(/\s+/g, '-')}`}
              ...
              >
                <div className="absolute inset-0 bg-gradient-to-br from-pink-600 to-purple-800 opacity-10 group-hover:opacity-20 transition-opacity duration-500"></div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-20">
        <div className="text-center mb-12">
          ...
          <h2 className="text-3xl md:text-4xl font-bold text-white mb-4">Featured Sportswear</h2>
          <p className="text-gray-300 text-lg max-w-2xl mx-auto">Our most popular women's sportswear items</p>
        </div>

        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          {womenSportswearProducts.slice(0, 8).map((product) => (
            <Link
              key={product.id}
              href={`/shop/women/sportswear/${product.type.toLowerCase().replace(/\s+/g, '-')}/${product.id}`}
...
export default WomenSportswearPage
