...
export default function WomenSportswearPage() {
  ...
  const filteredProducts = products.filter(p => p.subcategory === 'sportswear' && p.category === 'women')
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-black to-gray-900 pt-24">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-8">
          ...
          <h1 className="text-4xl font-bold text-white">Women's Sportswear</h1>
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
            {filteredProducts.map((product) => (
              <Link
                key={product.id}
                href={`/shop/sportswear/women/${product.id}`}
...
<div className="text-falco-accent font-bold">{formatPrice(product.price)}</div>
          ...
          <div className="text-center py-12 bg-gray-800/50 rounded-2xl border border-gray-700">
            ...
            <p className="text-gray-400">Check back soon for women's sportswear!</p>
          </div>
        )}
      </div>
    </div>
