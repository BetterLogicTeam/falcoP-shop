...
export default function MenShoesPage() {
  ...
  const filteredProducts = products.filter(p => p.subcategory === 'shoes' && p.category === 'men')
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-black to-gray-900 pt-24">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-8">
          ...
          <h1 className="text-4xl font-bold text-white">Men's Shoes</h1>
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
            {filteredProducts.map((product) => (
              <Link
                key={product.id}
                href={`/shop/shoes/men/${product.id}`}
          ...
          <div className="text-center py-12 bg-gray-800/50 rounded-2xl border border-gray-700">
            ...
            <p className="text-gray-400">Check back soon for men's shoes!</p>
          </div>
        )}
      </div>
    </div>
