...
const ProductDetailPage = () => {
  ...
  const handleWishlist = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        toast.error(data.error === 'Product already in wishlist' ? 'Already in wishlist' : data.error || 'Failed to add')
        return
      }
            ...
            <div className="space-y-8">
              ...
              <div className="flex space-x-4">
                ...
                <button
                  ...
                  className="p-4 border border-gray-600 text-white rounded-full hover:border-falco-accent hover:text-falco-accent transition-colors duration-300 disabled:opacity-50"
                ...
                <button
                  type="button"
                  onClick={handleShare}
                  className="p-4 border border-gray-600 text-white rounded-full hover:border-falco-accent hover:text-falco-accent transition-colors duration-300"
                  aria-label="Share"
                >
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                {relatedProducts.map((relatedProduct) => (
                  <Link
                    ...
                    href={`/shop/${relatedProduct.subcategory}/${relatedProduct.category}/${relatedProduct.id}`}
                    className="group bg-gradient-to-br from-gray-800 to-gray-900 rounded-xl border border-gray-700 hover:border-falco-accent transition-all duration-500 overflow-hidden hover:scale-105"
                  >
                    <div className="aspect-square bg-gray-700 relative overflow-hidden">
                      ...
                      {relatedProduct.badge && (
                        <div className="absolute top-3 left-3 bg-falco-accent text-black px-3 py-1 rounded-full text-xs font-bold">
                          {relatedProduct.badge}
                        </div>
                      )}
                    </div>
