...
import { ArrowLeft, Filter, Grid, List, Star, ShoppingBag, Users } from 'lucide-react'
...
const MenCategoryPage = () => {
  ...
  const sportswearProducts = menProducts.filter(p => p.subcategory === 'sportswear')
  const shoesProducts = menProducts.filter(p => p.subcategory === 'shoes')
  ...
  const subcategories = [
    {
      ...
      description: t('shop.men_sportswear_desc', 'High-performance athletic wear for men'),
    ...
    },
    {
      ...
      description: t('shop.men_shoes_desc', 'Premium footwear for every activity'),
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-black to-gray-900">
          ...
          <div className="text-center">
            <div className="flex justify-center mb-6">
              <div className="relative">
                <Users className="w-16 h-16 text-falco-accent drop-shadow-lg" />
                <div className="absolute -inset-4 bg-falco-accent/20 rounded-full blur-xl"></div>
              </div>
            </div>
            <h1 className="text-4xl md:text-6xl font-bold text-white mb-6 drop-shadow-lg">
              <span className="gradient-text">{t('footer.men', 'Men')}</span> {t('nav.shop', 'Shop')}
            </h1>
            <p className="text-xl text-gray-300 mb-8 max-w-3xl mx-auto leading-relaxed">
              {t('shop.men_hero', 'High-performance gear designed for the modern male athlete. From training essentials to competition wear, engineered for champions.')}
            </p>
            <div className="flex items-center justify-center space-x-8 text-sm text-gray-400">
              <div className="flex items-center space-x-2 bg-white/5 px-4 py-2 rounded-full backdrop-blur-sm">
                ...
                <span>{menProducts.length}+ {t('shop.products', 'Products')}</span>
              </div>
              <div className="flex items-center space-x-2 bg-white/5 px-4 py-2 rounded-full backdrop-blur-sm">
        ...
        <div className="grid md:grid-cols-2 gap-8">
          {subcategories.map((subcategory) => (
            <Link
              ...
              href={`/shop/men/${subcategory.slug}`}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-20">
        <div className="text-center mb-12">
          ...
          <h2 className="text-3xl md:text-4xl font-bold text-white mb-4">Featured Men's Products</h2>
          <p className="text-gray-300 text-lg max-w-2xl mx-auto">Our most popular men's items, trusted by athletes worldwide</p>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          {menProducts.slice(0, 8).map((product) => (
            <Link
              key={product.id}
              href={`/shop/men/${product.subcategory}/${product.type.toLowerCase().replace(/\s+/g, '-')}/${product.id}`}
