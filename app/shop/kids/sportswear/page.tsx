...
const KidsSportswearPage = () => {
  const { t } = useClientTranslation()
  const [viewMode, setViewMode] = useState<'grid' | 'list'>('grid')
  const [sortBy, setSortBy] = useState('featured')
  const [priceRange, setPriceRange] = useState([0, 500])

  // Filter products for kids sportswear
  const kidsSportswearProducts = products.filter(p => p.category === 'kids' && p.subcategory === 'sportswear')

  // Get unique types for this subcategory
  const productTypes = Array.from(new Set(kidsSportswearProducts.map(p => p.type)))
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-black to-gray-900">
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            ...
            >
              ...
              <span className="font-semibold">{t('common.back', 'Back to Kids')}</span>
            </Link>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative">
          <div className="text-center">
            ...
            <p className="text-xl text-gray-300 mb-8 max-w-3xl mx-auto leading-relaxed">
              {t('shop.kids_sportswear_desc', 'Fun and functional gear for active kids. Comfortable, durable, and designed for growing athletes.')}
            </p>
            <div className="flex items-center justify-center space-x-8 text-sm text-gray-400">
              <div className="flex items-center space-x-2 bg-white/5 px-4 py-2 rounded-full backdrop-blur-sm">
                ...
                <span>{kidsSportswearProducts.length}+ {t('shop.products', 'Products')}</span>
              </div>
              <div className="flex items-center space-x-2 bg-white/5 px-4 py-2 rounded-full backdrop-blur-sm">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-16">
        <div className="text-center mb-12">
          ...
          <p className="text-gray-300 text-lg max-w-2xl mx-auto">Find the perfect sportswear for your little athlete</p>
        </div>

        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {productTypes.map((type) => {
            const typeProducts = kidsSportswearProducts.filter(p => p.type === type)
            return (
              <Link
                key={type}
                href={`/shop/kids/sportswear/${type.toLowerCase().replace(/\s+/g, '-')}`}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-20">
        <div className="text-center mb-12">
          ...
          <p className="text-gray-300 text-lg max-w-2xl mx-auto">Our most popular kids' sportswear items</p>
        </div>

        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          {kidsSportswearProducts.slice(0, 8).map((product) => (
            <Link
              key={product.id}
              href={`/shop/kids/sportswear/${product.type.toLowerCase().replace(/\s+/g, '-')}/${product.id}`}
...
export default KidsSportswearPage
